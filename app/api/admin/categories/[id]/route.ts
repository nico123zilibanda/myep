...
export async function GET(_: Request, { params }: Params) {
  try {
    ...
    if (!user || user.role !== "ADMIN")
      return NextResponse.json({ success: false, messageKey: "UNAUTHORIZED" satisfies MessageKey}, 
    { status: 401 });

    if (isNaN(id))
      return NextResponse.json({ success: false, messageKey: "CATEGORY_NOT_FOUND" satisfies MessageKey}, 
    { status: 404 });
    ...
    console.error(err);
    return NextResponse.json({ success: false, messageKey: "SERVER_ERROR" satisfies MessageKey}, 
      { status: 500 });
  }
}
...
export async function PATCH(req: Request, { params }: Params) {
  try {
    ...
    if (error || !data)
      return NextResponse.json({ success: false, messageKey: "ACTION_FAILED" }, { status: 500 });
